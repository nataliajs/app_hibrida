...
function obtenerCoordenadas($parametros)
{
    ...
    $long = (string) $xml->result[0]->geometry->location->lng;
    
    $query = 'http://maps.googleapis.com/maps/api/elevation/json?locations='.$lat.','.$long.'&sensor=false';
    $jsonAlt = file_get_contents($query);
    $respAlt = json_decode($jsonAlt);
    $alt = $respAlt->results[0]->elevation;
    
    $respuesta->assign("latitud", "value", $lat);
    $respuesta->assign("longitud", "value", $long);
    $respuesta->assign('altitud' , 'value' , $alt);
